import numpy as np
from misc import generate_gc, generate_X_gc

class MUSCL_reconstruction:
    def __init__(self):
        self.NG = 2

    def update(self, U, X, dx, bc_type):
        self.U_gc = generate_gc(U, self.NG, bc_type)
        ...
        self.UL_gc, self.UR_gc = self.get_boudary()
    ...
    def get_mp(self):
        ...
        slope_plus = np.zeros_like(self.U_gc)
        slope_plus[:-1] = (self.U_gc[1:] - self.U_gc[:-1]) / dX[:, np.newaxis]
    ...
    def get_mm(self):
        ...
        slope_minus = np.zeros_like(self.U_gc)
        slope_minus[1:] = (self.U_gc[1:] - self.U_gc[:-1]) / dX[:, np.newaxis]

        return slope_minus
    
    def get_boudary(self):
        U_L_gc = self.U_gc - 0.5 * self.dx_gc[:, np.newaxis] * self.slope # left of cell i
        U_R_gc = self.U_gc + 0.5 * self.dx_gc[:, np.newaxis] * self.slope # right of cell i

        return U_L_gc, U_R_gc 
